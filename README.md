# Laravel13.x公式ドキュメント翻訳リポジトリ

このリポジトリはPHP Webアプリフレームワーク、Laravel13.xの公式英文ドキュメントを日本語へ翻訳しています。ドキュメントはreadouble.comでも、公開しています。

This is a Japanese translation repository for Laravel 13.x official documentations.
...
* このリポジトリでは13.xの翻訳和文のみ管理しています。readouble.comで公開している他のバージョンの和文に関しては、@HiroKwsにツイートいただくか、hirokws@gmail.comへご連絡ください。なお、和文ドキュメントのメンテナンス期間をフレームワーク本体のバグフィックス期間と合わせているため、readouble.comで公開している日本語翻訳の旧バージョンは更新していません。現在のバージョン13.xリリース時点で、メンテ対象の和文バージョンはこの13.x（２０２7年第３四半期まで）と12.x（２０２６年８月１３日まで）です。

#### issueで報告する場合

Issueで報告していただくのが、一番簡単です。

<https://github.com/laravel-ja/ja-docs-13.x/issues>のページで、"New issue"ボタンをクリックし、issueを発行してご報告ください。
...
原則、Laravel13.xのメンテ（バグフィックス）期間である１年６ヶ月間。
