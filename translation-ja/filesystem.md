...
`local`ドライバは、Laravelアプリケーションを実行しているサーバでローカルに保存しているファイルを操作し、`sftp`ストレージドライバはSSHキーベースのFTPを利用し、`s3`ドライバはAmazonのS3クラウドストレージサービスへの書き込みに使用します。
...
> [!WARNING]
> 一時的なアップロードURLの生成機能は、`s3`とローカルドライバのみサポートしています。
...
`files`メソッドは指定ディレクトリ内のすべてのファイルの配列を返します。サブディレクトリを含む指定ディレクトリ内のすべてのファイルのリストを取得したい場合は、`allFiles`メソッドを使用します。
...
`directories`メソッドは指定ディレクトリ内のすべてのディレクトリの配列を返します。サブディレクトリを含む、指定ディレクトリ内のすべてのディレクトリのリストを取得したい場合は、`allDirectories`メソッドを使用します。
