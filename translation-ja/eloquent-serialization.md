...
モデルの配列やJSON表現に含まれる、パスワードなどの属性を制限したい場合があります。その場合は、モデルで`Hidden`属性を使用します。`Hidden`属性にリストした属性は、モデルをシリアル化した表現に含みません。
...
> [!NOTE]
> リレーションシップを非表示にするには、Eloquentモデルの`Hidden`属性にリレーションシップのメソッド名を追加してください。

代わりに、`Visible`属性を使用して、モデルの配列やJSON表現に含めるべき属性の「許可リスト」を定義することもできます。モデルを配列やJSONに変換する際、`Visible`属性に存在していないすべての属性は非表示になります。
...
アクセサを常にモデルの配列やJSON表現へ追加したい場合は、モデルで`Appends`属性を使用します。アクセサのPHPメソッドは「キャメルケース」で定義しますが、属性名は通常、シリアル化された表現である「スネークケース」を使って参照する点に注意してください。
