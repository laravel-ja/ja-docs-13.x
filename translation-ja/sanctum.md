...
ファーストパーティのステートフルドメインの設定を支援するため、Sanctumは設定に含めることができる２つのヘルパ関数を提供しています。まず、`Sanctum::currentApplicationUrlWithPort()`は、環境変数`APP_URL`から現在のアプリケーションのURLを返します。`Sanctum::currentRequestHost()`は、ステートフルなドメインリストへプレースホルダを挿入します。これは実行時に現在のリクエストのホストに置き換えられ、同じドメインからのすべてのリクエストがステートフルと見なされるようになります。
...
さらに、アプリケーションのグローバルな`axios`インスタンスで`withCredentials`と`withXSRFToken`オプションを有効にする必要があります。通常、これは`resources/js/app.js`ファイルで実行可能です。フロントエンドからHTTPリクエストを行うためにAxiosを使用していない場合は、独自のHTTPクライアントで同等の構成を実行する必要があります。
...
このSPA認証のアプローチはセッションベースであるため、["リメンバー・ミー"](/docs/{{version}}/authentication#remembering-users)機能を含む、Laravelの標準的な認証サービスを使用できます。
...
利用開始するには、ユーザーの電子メール/ユーザー名、パスワード、およびデバイス名を受け入れるルートを作成し、それらの資格情報を新しいSanctumトークンと交換します。このエンドポイントに付けられた「デバイス名」は情報提供を目的としたものであり、任意の値にすることができます。一般に、デバイス名の値は、「Nuno'siPhone17」などのユーザーが認識できる名前である必要があります。
