...
- [Laravelアプリの生成](#creating-a-laravel-project)
    - [AIを使い開始する](#getting-started-using-ai)
...
[Claude Code](https://docs.anthropic.com/en/docs/claude-code)や[OpenCode](https://opencode.ai)のようなAIコーディングエージェントを使用している場合は、プロジェクトに手を加える前に、エージェントにLaravel固有のプレイブックを与えるプロンプトから開始できます。

以下のプロンプトは、Laravelのインストールガイダンスの場所、優先すべき事項、および選択が行われていない場合に適切なデフォルト設定を作成する方法をエージェントに伝えます。開始する時に、これをエージェントへ貼り付けてください。
...
エージェントが指示を読み取った後、ステップバイステップであなたを導き、セットアップをLaravelのデフォルトに合わせるはずです。
