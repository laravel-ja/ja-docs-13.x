...
- [セッションのシリアライズ設定](#session-serialization-configuration)
...
LaravelのデフォルトのキャッシュとRedisキーのプレフィックスは、ハイフンでつながれたサフィックスを使用するようになりました。
...
PHPデシリアライゼーションガジェットチェーン攻撃を防ぎやすくするため、デフォルトのアプリケーションスケルトンでは、`config/session.php`ファイル内のセッションの`serialization`オプションを`json`に設定するようになりました。

既存のアプリケーションをアップグレードし、設定ファイルをLaravel 13スケルトンと同期する場合、この値を`php`から`json`に更新すると、すべてのアクティブなユーザーセッションが無効になります。

アップグレード中もアクティブなセッションをシームレスに維持したい場合は、この値を`php`のまま維持してください。ただし、アプリケーションがセッションにPHPオブジェクトを保存しておらず、ユーザーに再認証を求めても問題がない場合は、セキュリティ向上のためにこの値を`json`に更新することをお勧めします。
...
#### Symfony PHP8.5ポリフィルとグローバル関数の競合

**影響の可能性： 低い**

Laravel 13は、`symfony/polyfill-php85`への依存を導入しました。PHP8.5未満のバージョンでは、このポリフィルは、ブートストラップ中にあらかじめ定義されていない限り、`array_first()`や`array_last()`といったグローバル関数を定義します。

これらの関数は、`laravel/helpers`のようなレガシーなヘルパパッケージや、同じ名前を使用しているカスタムグローバルヘルパと競合する可能性があります。たとえば、歴史的な`array_first()`ヘルパは、最初に一致する要素を返すためのコールバックを受け入れましたが、ポリフィル版は配列の最初の要素を返すだけです。

競合を避け、PHPのバージョンを跨いで一貫した動作を保証するために、`Illuminate\Support\Arr`メソッドの使用を推奨します。
