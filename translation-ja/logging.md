...
Laravel Pail（ペール：バケツ、手杓）は、Laravelアプリケーションのログファイルにコマンドラインから直接簡単にアクセスできるパッケージです。標準の`tail`コマンドとは異なり、Pailは[Laravel Nightwatch](https://nightwatch.laravel.com)、Sentry、Flareを含むあらゆるログドライバで動作するように設計されています。さらにPailは、探しているログをすぐに見つけのに役立つ、便利なフィルタのセットを用意しています。
