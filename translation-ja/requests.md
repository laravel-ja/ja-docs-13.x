...
`TrustHosts`ミドルウェアを有効にするには、アプリケーションの`bootstrap/app.php`ファイル内で、`trustHosts`ミドルウェアメソッドを呼び出します。このメソッドの`at`引数を使用し、アプリケーションが応答すべきホスト名を指定します。ホスト名の文字列は正規表現として扱います。これ以外の`Host`ヘッダを持つ受信リクエストは拒否します。
