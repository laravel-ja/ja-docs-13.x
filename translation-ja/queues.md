...
- Redis: `predis/predis ~3.0`またはphpredis PHP拡張機能
...
ジョブのバッチをディスパッチするには、`Bus`ファサードの`batch`メソッドを使用する必要があります。もちろん、バッチ処理は主に完了コールバックと組み合わせる場合に役立ちます。したがって`then`、`catch`、`finally`メソッドを使用して、バッチの完了コールバックを定義できます。これらの各コールバックは、呼び出されたときに`Illuminate\Bus\Batch`インスタンスを受け取ります。

複数のキューワーカを実行する場合、バッチ内のジョブを並列処理します。そのため、ジョブの完了順序はバッチの追加順序と一致しない場合があります。一連のジョブを順番に実行する方法には、[ジョブチェーンとバッチ](#chains-and-batches)に関するドキュメントを参照してください。

この例では、CSVファイルから指定行数を処理するジョブのバッチをキューに追加するケースを想定しています。
...
すべての接続のすべてのキューでジョブ処理を一時停止するには、`--all`オプションを使用します。
...
すべての接続のすべてのキューでジョブ処理を再開するには、`queue:resume`コマンドで`--all`オプションを使用します。

```shell
php artisan queue:resume --all
```

キューを再開すると、ワーカはそのキューから新しいジョブの処理を直ちに開始します。すべてのキューを再開しても、個別に一時停止したキューは再開しません。キューを一時停止してもワーカプロセス自体は停止せず、指定したキューからワーカが新しいジョブを処理するのを防ぐだけである点に注意してください。
