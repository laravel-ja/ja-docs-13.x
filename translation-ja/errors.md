...
> [!WARNING]
> 本番環境では、`APP_DEBUG`の値は常に`false`であるべきです。もし本番環境でこの値が`true`に設定されていると、アプリケーションのエンドユーザーに対して機密性の高い設定値をさらけ出してしまうリスクがあります。**
...
内部的に、Laravelは404 HTTPエラーによる例外、オリジンの不一致により生成される403 HTTPレスポンス、または無効なCSRFトークンにより生成される419 HTTPレスポンスなど、いくつかのタイプのエラーをあらかじめ無視します。特定のタイプの例外を無視するのをやめるようLaravelに指示したい場合は、アプリケーションの`bootstrap/app.php`ファイルで、例外の`stopIgnoring`メソッドを使用します。
