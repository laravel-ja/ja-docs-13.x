...
React、Svelte、Vue、Livewireのスターターキットは、チームサポート付きで生成することもできます。チーム機能が有効な場合、各ユーザーは１つ以上のチームに所属し、現在のチームを持ちます。登録時に、新ユーザーは自動的に個人チームを割り当てます。スターターキットには、チームの作成、チームの切り替え、メンバーの招待、チーム詳細の更新を行うためのチーム管理画面も用意しています。

ルートが現在のチームにスコープされている場合、URLには現在のチームのスラッグが含まれます。たとえば、ダッシュボードのルートは`/{current_team}/dashboard`になり、チーム管理ページは`settings/teams/{team}`のようなルートを使用します。`{current_team}`および`{team}`ルートパラメータを使用する場合、スターターキットはルートへのアクセスを許可する前に、認証済みユーザーがリクエストしたチームに所属していることを自動的に確認します。

チームを意識したURLの生成を便利にするために、スターターキットは認証済みユーザーの現在のチームに対してURLのデフォルト値を登録します。これにより、`route('dashboard')`などのヘルパへの呼び出しに、現在のチームのスラッグを自動的に含めることができます。ユーザーがサインイン、登録、またはチームを切り替えると、スターターキットは現在のチームを更新し、これらのURLデフォルト値をリフレッシュするため、生成したリンクは正しいチームコンテキストを引き続き使用します。

チームを作成または名前変更するとき、スターターキットは、安全でない、または競合するルートセグメントを生成する可能性のある予約済み名称をユーザーが選択するのを防ぎます。たとえば、`settings`、`login`、`dashboard`などのルートプレフィックスと衝突する名前は使用できません。
